...
import { ShieldAlert, Server, Users, Calendar, X, ExternalLink } from "lucide-react";
  ...
  return (
    <div className="space-y-8 pb-8">
        ...
        <div className="relative bg-gradient-to-r from-primary to-primary/80 rounded-2xl p-6 md:p-8 text-primary-foreground shadow-lg overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-primary-foreground/80 max-w-2xl mb-6">
              You are logged in as <strong className="text-white">{user?.role}</strong>. 
              The Priority-Based Lab Management System (PLMS) resolves conflicts automatically.
            </p>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl font-display font-bold tracking-tight">Live Schedule Overview</h2>
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold">{isLoading ? "-" : displayStats.totalLabs}</h3>
              </div>
              <div className="p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl">
                <Server className="w-5 h-5" />
              </div>
            </div>
            <p className="text-sm text-muted-foreground mt-4">{displayStats.totalCapacity} total seats</p>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold">{isLoading ? "-" : displayStats.occupancyPercent}%</h3>
              </div>
              <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-xl">
                <Users className="w-5 h-5" />
              </div>
            </div>
            <p className="text-sm text-muted-foreground mt-4">{displayStats.labsInUse} labs currently in use</p>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold text-red-600 dark:text-red-400">{isLoading ? "-" : displayStats.pendingConflicts}</h3>
              </div>
              <div className="p-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-xl">
                <ShieldAlert className="w-5 h-5" />
              </div>
            </div>
            <p className="text-sm text-muted-foreground mt-4">{displayStats.autoResolved} auto-resolved today</p>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold">{isLoading ? "-" : displayStats.availableNow}</h3>
              </div>
              <div className="p-3 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 rounded-xl">
                <Calendar className="w-5 h-5" />
              </div>
            </div>
            <p className="text-sm text-muted-foreground mt-4 text-green-600 dark:text-green-400 font-medium">Ready for booking</p>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="p-6">
          <div className="grid grid-cols-5 gap-4">
            ...
            <div className="priority-p1 rounded-xl p-4 shadow-sm relative overflow-hidden group">
              <div className="absolute top-0 right-0 p-2"><span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-red-200 text-red-800 dark:bg-red-900/50">P1</span></div>
            ...
            <div className="priority-p3 rounded-xl p-4 shadow-sm relative overflow-hidden">
              <div className="absolute top-0 right-0 p-2"><span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-emerald-200 text-emerald-800 dark:bg-emerald-900/50">P3</span></div>
            ...
            <div className="priority-p2 rounded-xl p-4 shadow-sm relative overflow-hidden">
              <div className="absolute top-0 right-0 p-2"><span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-amber-200 text-amber-800 dark:bg-amber-900/50">P2</span></div>
              <p className="text-sm font-semibold mb-1">OS Lab Practical</p>
              <p className="text-xs opacity-80">Lab 301 • 13:00 - 16:00</p>
            </div>
            <div className="border-2 border-dashed border-border rounded-xl p-4 flex flex-col items-center justify-center text-muted-foreground hover:bg-muted/50 cursor-pointer transition-colors" onClick={() => setNewBookingOpen(true)}>
